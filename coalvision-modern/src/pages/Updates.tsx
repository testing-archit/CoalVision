...
import { getUpdates } from '../services/database';
...
interface Update {
    ...
    production_increase: string;
    air_quality: string;
    water_usage: string;
    community_projects: string;
    community_workshops: string;
    created_at: string;
}
                        ...
                        {updates.map((update, index) => (
                            ...
                            >
                                <div className="flex items-start justify-between mb-4">
                                    ...
                                    <span className="text-sm text-slate-400">{formatDate(update.created_at)}</span>
                                </div>
                                ...
                                <div className="p-4 bg-blue-500/10 rounded-lg border-l-4 border-blue-500 mb-4">
                                    ...
                                    <div className="grid grid-cols-2 gap-4 text-sm">
                                        <p className="text-slate-300">
                                            <span className="text-slate-400">Workers:</span> {update.worker_count?.toLocaleString()}
                                        </p>
                                        <p className="text-slate-300">
                                            <span className="text-slate-400">Safety Training:</span> {update.safety_training}
                                        </p>
                                    </div>
                                </div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                    ...
                                    <div className="p-4 bg-slate-700/50 rounded-lg">
                                        ...
                                        <p className="text-sm text-slate-300">Total: {update.production_total}</p>
                                        <p className="text-sm text-emerald-400">+{update.production_increase} Monthly</p>
                                    </div>
                                    ...
                                    <div className="p-4 bg-slate-700/50 rounded-lg">
                                        ...
                                        <p className="text-sm text-slate-300">Air Quality: {update.air_quality}</p>
                                        <p className="text-sm text-slate-300">Water: {update.water_usage}</p>
                                    </div>
                                    ...
                                    <div className="p-4 bg-slate-700/50 rounded-lg">
                                        ...
                                        <p className="text-sm text-slate-300">{update.community_projects}</p>
                                        <p className="text-sm text-slate-300">{update.community_workshops}</p>
                                    </div>
                                </div>
                            </motion.div>
                        ))}
